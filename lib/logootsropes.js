...
function LogootSRopes (replicaNumber) {
    console.assert(arguments.length === 0 || typeof replicaNumber === "number", "replicaNumber = ", replicaNumber);
...
LogootSRopes.prototype.addBlock = function (args) {
    ...
    if(args.hasOwnProperty("idi")) {
        ...
        console.assert(idi instanceof IdentifierInterval,
            "args.idi = ", idi);
        console.assert(from instanceof RopesNodes,
            "args.from = ", from);
        console.assert(typeof startOffset === "number",
        ...
        if (this.root === null) {
            var bl = new LogootSBlock(idi2, 0);
...
LogootSRopes.prototype.toString = function () {
    var treeRepr = (this.root !== null) ? this.root.toString() : "";
    var digest = 11;

    for (var i = 0; i < treeRepr.length; i++) {
        digest = (31 * digest) + treeRepr.charCodeAt(i);
        digest = digest|0; // COnvert to 32bits integer
    }
