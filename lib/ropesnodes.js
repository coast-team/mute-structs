...
RopesNodes.fromJSON = function (node) {
 	return new RopesNodes({
		...
		block: node.hasOwnProperty("block") ? LogootSBlock.fromJSON(node.block) : null,
		left: node.hasOwnProperty("left") ? RopesNodes.fromJSON(node.left) : null,
		right: node.hasOwnProperty("right") ? RopesNodes.fromJSON(node.right) : null,
	});
};
